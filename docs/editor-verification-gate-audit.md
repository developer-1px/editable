...
| 항목 | 현재 결과 | 판정 |
| --- | --- | --- |
| focused/skipped/todo marker scan | 38 test files, violation 없음 | focused/skip/todo/conditional/expected-failure marker가 현재 테스트 기준선을 약하게 만들지 않는다. |
| Vitest discovery parity | `vitest list --filesOnly --json` 38 files, marker scan 38 files, 차이 없음 | 현재 Vitest가 발견하는 test file 집합과 marker scan 대상이 일치한다. |
| `verify:docs` | README Docs covers 89 docs files; evidence strength covers 87 editor docs | docs file/link inventory와 editor evidence-section coverage는 현재 통과한다. |
| `verify:boundaries` | violation output 없음 | public/internal seam과 import direction은 현재 통과한다. |
| normal Vitest | 38 files, 863 tests passed | jsdom unit/integration baseline은 현재 통과한다. |
| seeded shuffled Vitest | seed `20260621`, 38 files, 863 tests passed | deterministic order baseline은 현재 통과한다. |
| Biome check | Checked 110 files | source/config/scripts/CSS formatting/lint/import organization baseline은 현재 통과한다. |
...
| type/test/build/check baseline | 확정 | Current `verify:internal -- --repeat=1` output이 test marker scan 38 files/no violations, Vitest discovery parity 38 files/no diff, typecheck, normal/shuffled Vitest 38 files/863 tests, Biome 110 files, client/SSR build, diff-check 통과를 확인한다. | Passing tests는 browser/AT matrix, release QA, dependency/security status를 대체하지 않는다. |
