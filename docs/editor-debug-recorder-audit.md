...
| event capture scope | recorder가 window에 붙이는 input event list는 beforeinput, key/mouse/pointer/composition/clipboard/wheel 중심의 23종이다. Keyboard event는 `keyCode`와 `isComposing`, input event는 `inputType`/`data`/`isComposing`, composition event는 `data`를 남긴다. Console capture는 `console.warn`/`console.error`만 patch한다. | `debugInteractionTypes.ts`, `debugInteractionEvents.ts`, `useDebugInteractionRecorder.ts`, `BlockEditor.test.tsx`, `debugInteractionSnapshot.test.ts` |
...
| clipboard event summary | recorder는 paste/copy 이벤트에서 실제 transfer reader가 선택한 clipboard text와 `DataTransfer.types` MIME 목록을 timeline에 기록한다. custom MIME이 plain text보다 우선하면 report도 structured text를 쓴다. | `debugInteractionEvents.ts`, `BlockEditor.test.tsx`, `debugInteractionSnapshot.test.ts` |
