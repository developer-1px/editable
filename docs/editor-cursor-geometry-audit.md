...
| empty and code block carets | empty paragraph, empty visual line after hard newline, code block padding, code block hard newline 위치가 caret rect를 가진다. | `cursorGeometry.test.ts` |
| coordinate hit testing | viewport coordinate를 nearest valid cursor point로 변환하고, scroll 후 현재 viewport rect를 다시 읽는다. invalid DOM에는 null을 반환한다. | `cursorGeometry.test.ts` |
| wrapped and hard-break rows | soft wrap, hard newline, hard-break whitespace hit testing, wrapped boundary affinity를 visual row 기준으로 처리한다. | `cursorGeometry.test.ts` |
...
| coordinate hit testing | 확정 | viewport coordinate to nearest cursor point, figure/text hit testing, scroll 후 current rect 재평가, invalid DOM null 반환이 테스트로 고정되어 있다. | Real browser 좌표 matrix, touch/pen event ordering은 아직 닫지 않았다. |
| vertical/page/line movement support | 확정 | ordered row vertical movement, preferredX preservation, geometry page step, line boundary 조회, figure/empty paragraph traversal이 view/model adapter tests에 있다. | Cross-browser visual movement parity와 platform shortcut policy는 별도 검증이 필요하다. |
