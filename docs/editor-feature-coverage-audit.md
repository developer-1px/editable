...
`richSelection.test.ts`, `cursorCommands.test.ts`, `cursor.test.ts`,
`inputAdapter.test.ts`, `BlockEditor.test.tsx`가 caret/range/node serialization,
...
닫고, `textCommands.test.ts`, `inputAdapter.test.ts`,
`contentEditableViewEngine.test.ts`, `BlockEditor.test.tsx`가 collapsed insert,
...
`contentEditableViewEngine.test.ts`, `BlockEditor.imeTrace.test.tsx`,
`BlockEditor.test.tsx`가 composition 중 model mutation 지연, final commit flush,
...
부분확정이다. ED-010, ED-021과 `clipboard.test.ts`,
`inputAdapter.test.ts`, `BlockEditor.test.tsx`가 collapsed copy no-op, text
...
확정이다. ED-002, ED-010, ED-018과 `cursor.test.ts`,
`cursorCommands.test.ts`, `inputAdapter.test.ts`가 ArrowLeft/Right,
...
부분확정이다. ED-025와 `cursor.test.ts`, `cursorCommands.test.ts`,
`inputAdapter.test.ts`가 Alt/Option+ArrowLeft/Right, Shift extension, decomposed
...
확정이다. ED-009, ED-024와 `cursorGeometry.test.ts`,
`cursorCommands.test.ts`, `inputAdapter.test.ts`, `BlockEditor.test.tsx`가
...
확정이다. ED-010, ED-024, ED-027과 `inputAdapter.test.ts`,
`cursorCommands.test.ts`, `cursorGeometry.test.ts`가 Home/End,
...
확정이다. ED-004, ED-005, ED-026과 `textCommands.test.ts`,
`inputAdapter.test.ts`가 Backspace/Delete, grapheme deletion, selected range
...
확정이다. ED-005, ED-021, ED-023과 `textCommands.test.ts`,
`inputAdapter.test.ts`, `blockCommands.test.ts`, `BlockEditor.test.tsx`가 Enter
...
`inputAdapter.test.ts`의 Tab outside list 회귀 테스트가 직접 고정한다.
...
부분확정이다. ED-015, ED-022, ED-028과 `markCommands.test.ts`,
`inputAdapter.test.ts`, `normalizer.test.ts`, `cursor.test.ts`,
`DocumentRenderer.test.tsx`가 bold/italic/code/link toggle, range mark
...
`textCommands.test.ts`, `editorRegressionScenarios.test.ts`,
`richSelection.test.ts`, `editorCore.test.ts`, `BlockEditor.test.tsx`가
...
부분확정이다. `BlockEditor.test.tsx`와 `blockEditorSelection.ts`가 text caret
...
확정이다. ED-007, ED-008, ED-011과 `DocumentRenderer.test.tsx`,
`CursorOverlay.test.tsx`, `SelectionOverlay.test.tsx`, `cursorGeometry.test.ts`,
`BlockEditor.test.tsx`가 canonical selection 기반 caret/range/selected atom
...
부분확정이다. ED-021, ED-029와 `inputAdapter.test.ts`,
`editorKeyboardPolicy.test.ts`, `BlockEditor.test.tsx`,
`contentEditableViewEngine.test.ts`가 macOS Cmd와 Windows/Linux Ctrl intent
