...
- 확정: paragraph, heading, quote, listItem 같은 inline text block에서는 block을
  둘로 나눈다.
...
보내고, `textCommands`는 inline text block과 codeBlock을 분리해 처리한다.
...
| `textCommands.ts` | inline text block은 block split, codeBlock은 text newline, atom edge는 인접 paragraph 생성으로 분기한다. |
