...
| `src/editor/internal/model/cursor.test.ts` | visible character cursor, grapheme, shared formatted text-run boundary, atom cursor unit을 검증한다. |
| `src/editor/internal/model/cursorCommands.test.ts` | horizontal/word/vertical/page/boundary movement, selectAll, selected atom render derivation을 검증한다. |
| `src/editor/internal/model/editorCore.test.ts` | `createEditor().query({ type: "selection" })`, selection-only dispatch no-history, geometry-required movement failure를 검증한다. |
...
| range normal form과 render atom derivation | 실행 테스트로 확정 | `richSelection.test.ts`, `cursorCommands.test.ts`, `inputAdapter.test.ts`가 source range의 empty `selectedPointers`와 render 단계 mention/figure atom derivation을 검증한다. |
...
| default/invalid selection normalization | 실행 테스트로 확정 | `editorCore.test.ts`가 missing initial selection과 invalid low-level snap이 첫 block before edge selection으로 수렴함을 검증한다. |
| selectAll과 movement context | 실행 테스트로 확정 | `cursorCommands.test.ts`와 `inputAdapter.test.ts`가 selectAll, range extension, preferredX, horizontal movement context clear를 검증한다. |
| selection-only no-history | 실행 테스트로 확정 | `editorCore.test.ts`가 selection-only dispatch가 `canUndo`를 만들지 않는다고 검증한다. |
| geometry adapter seam | 실행 테스트로 확정 | `editorCore.test.ts`가 geometry 없는 line movement failure와 view adapter가 있을 때의 visual movement success를 검증한다. |
