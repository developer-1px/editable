...
4. `verify:docs`, `verify:boundaries`, `verify:internal`로 README docs inventory,
   generated route tree stability, public/internal seam, 전체 baseline을 묶는다.
...
| Surface | 확정 범위 | 근거 |
| --- | --- | --- |
| editor state/model | `@interactive-os/json-document`는 canonical document/selection/history state이고, `zod`는 internal document schema validation이다. | `src/editor/internal/model/*`, `src/editor/internal/react/BlockEditor.tsx`, `docs/repo-analysis-report.md` |
| text geometry | `@chenglou/pretext`는 rich inline layout/geometry adapter에서 쓰인다. | `src/editor/internal/view/cursorGeometryLayout.ts` |
...
| route generation | 현재 Start app의 routeTree freshness는 `verify:internal`의 build wrapper가 확인한다. Build가 route tree를 바꾸면 원래 content를 복원하고 실패하는 동작은 script test로 고정한다. `generate-routes`/`tsr generate` standalone interface는 Start register tail을 빼므로 제거했다. | `package.json`, `src/routeTree.gen.ts`, `scripts/verify-internal.mjs`, `scripts/verify-internal.test.mjs` |
| test runtime | `vitest`, `jsdom`, `@testing-library/react`, `@testing-library/dom`은 jsdom React/editor tests에 필요하다. `@testing-library/dom`은 React Testing Library peer로 직접 유지한다. | `src/editor/**/*.test.*`, `pnpm why @testing-library/dom` |
...
| verification scripts | `verify:docs`, `verify:boundaries`, `verify:internal`, stress/soak scripts는 current repo analysis의 quality gate다. Docs inventory, boundary 대표 violation reporting, internal repeat parsing, route tree restore/fail은 script tests로 고정한다. Boundary test는 static/type-only/export-from/dynamic/commented-dynamic/current-Vite-glob/require/import-equals/import-type hidden import leak, public facade arbitrary helper/canonical-name/namespace/star/star-as leak, React facade headless/non-react/arbitrary-helper/canonical-name/star/star-as leak, runtime test-only import blocking, test-file allowance, testing helper implementation import blocking, fixture non-testing import blocking을 포함한다. | `package.json`, `scripts/verify-docs-inventory.mjs`, `scripts/verify-docs-inventory.test.mjs`, `scripts/verify-editor-boundaries.mjs`, `scripts/verify-editor-boundaries.test.mjs`, `scripts/verify-internal.mjs`, `scripts/verify-internal.test.mjs`, `docs/editor-verification-gate-audit.md` |
...
| Command | 결과 | 판정 |
| --- | --- | --- |
| `pnpm list --depth=0` | direct production dependencies 10개, direct devDependencies 11개. 제거 대상 devtools/query/typography/router-cli/browser-provider package는 top-level direct dependency로 나타나지 않는다. | 현재 package manifest의 직접 surface는 editor/runtime/build/test/gate에 필요한 항목으로 좁혀져 있다. |
| `pnpm audit --json` | total dependencies 257, low/moderate/high/critical 0건 | 현재 installed graph의 known vulnerability evidence는 clean이다. |
...
| 항목 | 강도 | 이유 |
| --- | --- | --- |
| direct dependency inventory | 확정 | `package.json`과 `pnpm list --depth=0`가 production dependencies 10개, devDependencies 11개를 같은 top-level package surface로 보여준다. |
...
| current audit/license snapshot | 확정 snapshot | 현재 실행 기준 `pnpm audit --json`은 total dependencies 257, vulnerability 0건이고 `pnpm audit --prod --json`은 production dependencies 163, vulnerability 0건이다. `pnpm licenses list --json`은 207 entries inventory를 제공하며 license counts는 MIT 174, ISC 10, Apache-2.0 6, BSD-3-Clause 4, BSD-2-Clause/MIT OR Apache-2.0/MIT-0/MPL-2.0 각 2, BlueOak-1.0.0/CC-BY-4.0/CC0-1.0/Python-2.0/Unlicense 각 1이다. |
...
| browser smoke dependency/gate | 미정 | `@vitest/browser-playwright` dependency path와 package script는 없다. 기존 환경의 일회성 preview/Chrome smoke는 확인했지만 자동 browser QA gate는 아니다. |
...
| browser smoke package | Playwright/Vitest browser provider는 현재 direct dependency나 package script가 아니다. `pnpm-lock.yaml`의 `@vitest/browser-playwright` 표기는 `vitest@4.1.9` optional peer metadata이고, `pnpm list --depth=0`과 `pnpm why @vitest/browser-playwright`에는 installed dependency path가 없다. Vite preview + curl server smoke와 일회성 Chrome headless CDP smoke는 기존 환경으로 확인했지만 browser smoke는 아직 package script나 gate가 아니다. | browser QA를 제품 gate로 둘 때 추가한다. |
...
pin policy, dependency/security gate policy, browser smoke dependency/gate 추가 여부다. 현재 installed
...
smoke는 기존 환경으로 확인했지만 browser smoke dependency/gate는 아직 추가하지 않는다.
