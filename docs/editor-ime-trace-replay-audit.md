...
다만 이 helper는 실제 OS/browser IME matrix나 debug recorder report replay
...
| 경로 | 확정 동작 | 근거 |
| --- | --- | --- |
| internal 위치 | replay helper와 fixture는 `src/editor/internal/testing`, `src/editor/internal/fixtures/ime` 아래에 있고 public/react facade export가 아니다. | `editorTraceReplay.ts`, `koreanHangulBasicTrace.ts`, `koreanHangulEnterConfirmTrace.ts`, `scripts/verify-editor-boundaries.mjs` |
| trace schema | fixture shape는 `schema: "editable-trace-replay@1"` literal type으로 고정되어 있다. | `editorTraceReplay.ts`, IME fixture files |
| source inventory | current trace surface는 replay helper 1개와 fixture 2개다. Adjacent composition trace는 test 안의 inline fixture로 남아 있다. | `rg --files src/editor/internal/fixtures/ime src/editor/internal/testing`, `BlockEditor.imeTrace.test.tsx` |
| replay interface | `replayEditorTrace(root, trace)`는 replayed events의 `defaultPrevented` 결과 배열을 반환하고, `findReplayedEvent`는 type/inputType으로 그 결과를 찾는다. | `editorTraceReplay.ts`, `BlockEditor.imeTrace.test.tsx` |
| replay steps | 지원 step은 `event`, `selection`, `text`, `timers`다. selection/text step은 `data-path` text run을 직접 조작하고, event step은 keyboard/composition/input event만 dispatch한다. Pointer, clipboard, drop, focus/blur replay는 없다. | `editorTraceReplay.ts` |
| event result | replay는 각 event의 `defaultPrevented`를 반환해서 final commit `beforeinput insertText`가 editor에 의해 막혔는지 검증하게 한다. | `editorTraceReplay.ts`, `BlockEditor.imeTrace.test.tsx` |
...
| stale composition end | 첫 Hangul composition이 끝나자마자 다음 composition이 시작될 때 stale timer가 새 preedit을 release하지 않는 것을 검증한다. | `BlockEditor.imeTrace.test.tsx`, `contentEditableViewEngine.test.ts` |
| Enter confirmation | IME confirmation Enter가 paragraph split으로 처리되지 않고 final composition commit으로만 남는 것을 검증한다. | `koreanHangulEnterConfirmTrace.ts`, `BlockEditor.imeTrace.test.tsx` |
...
| 강도 | 해당 항목 | 현재 의미 |
| --- | --- | --- |
| test interface 확정 | `EditorTraceReplay` schema, `event`/`selection`/`text`/`timers` step union, `replayEditorTrace` return shape, fixture 2개 | jsdom React regression test가 배워야 하는 replay interface다. Public editor caller가 배워야 하는 interface는 아니다. |
| 실행 테스트로 닫힘 | Korean starter key duplicate commit 방지, adjacent composition stale timer 방지, Enter confirmation no split, final commit `defaultPrevented` assertion | 현재 regression gate가 직접 잡는 IME trace replay 기준선이다. |
...
| replay event 범위 확장 | 현재 replay event는 keyboard/composition/input에 한정된다. Clipboard, drop, focus/blur, pointer/selectionchange까지 같은 fixture schema로 넓힐 근거는 아직 없다. | 실제 회귀가 생기면 기존 helper에 좁게 추가할지, 별도 browser trace format으로 분리할지 먼저 결정한다. |

## /doubt 판정

| 항목 | 판정 | 이유 |
| --- | --- | --- |
| `editorTraceReplay` helper | 유지 확정 | IME event, DOM preedit text, timers, default-prevented assertion을 하나의 internal replay interface로 묶는다. 삭제하면 같은 회귀를 테스트마다 장황하게 다시 작성해야 한다. |
| Korean Hangul fixtures | 유지 확정 | starter key duplicate commit과 Enter confirmation은 jsdom 단위 이벤트보다 실제 browser event order에 가까운 회귀 근거다. |
...
keyboard/composition/input event 재생, Korean composition duplicate commit 방지,
stale composition end 방지, Enter confirmation 처리, test-only boundary다. 반대로 실제
browser/OS IME matrix, trace capture/import pipeline, debug recorder 호환성, replay event
범위 확장, external JSON validation은 아직 제품/운영/QA 결정으로 남긴다.
