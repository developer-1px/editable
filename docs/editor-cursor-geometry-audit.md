...
| 경로 | 확정 동작 | 근거 |
| --- | --- | --- |
| legal cursor stop rects | invariant fixtures의 모든 legal cursor stop이 finite caret/edge rect를 갖는다. | `cursorGeometry.test.ts` |
| text caret and range rects | text offset caret, selected text range, hard newline selection, zero-width caret visibility가 rect로 표현된다. | `cursorGeometry.test.ts`, `CursorOverlay.test.tsx`, `SelectionOverlay.test.tsx` |
| inline and block atom edges | mention atom과 figure block의 before/after edge rect를 만들고, range rect는 atom edge 자체를 text range처럼 invent하지 않는다. | `cursorGeometry.test.ts`, `SelectionOverlay.test.tsx` |
| model-order range rects | inline atom을 사이에 둔 range도 DOM order가 아니라 model order로 text rect를 만든다. block atom edge도 document order에 맞춘다. | `cursorGeometry.test.ts` |
| marked text DOM mapping | `strong`, `em`, `code`, link 같은 nested mark element 안에서도 visible offset을 text node path로 mapping한다. | `cursorGeometry.test.ts`, `DocumentRenderer.test.tsx` |
| empty and code block carets | empty paragraph, rendered empty text-run, empty visual line after hard newline, code block padding, code block hard newline 위치가 caret rect를 가진다. | `cursorGeometry.test.ts` |
| coordinate hit testing | viewport coordinate를 nearest valid cursor point로 변환하고, rendered empty paragraph와 hard newline이 만든 선행/중간/연속/후행 빈 visual row의 whitespace도 같은 row의 cursor point로 유지한다. scroll 후 현재 viewport rect를 다시 읽고 invalid DOM에는 null을 반환한다. | `cursorGeometry.test.ts` |
| wrapped and hard-break rows | soft wrap, hard newline, hard-break whitespace hit testing, hard newline이 만든 선행/중간/연속/후행 빈 visual row, wrapped boundary affinity를 visual row 기준으로 처리한다. | `cursorGeometry.test.ts` |
| vertical movement | ordered row movement가 current-line nearest hit testing으로 되돌아가지 않고, figure/empty paragraph/consecutive empty paragraph를 지나간다. | `cursorGeometry.test.ts`, `cursorCommands.test.ts`, `inputAdapter.test.ts` |
| page and line boundaries | `pageStep`은 root viewport height를 쓰고, line start/end geometry는 Home/End, Cmd/Ctrl+Arrow line movement에 쓰인다. | `cursorGeometry.test.ts`, `inputAdapter.test.ts`, `cursorCommands.test.ts` |
...
| 항목 | 판정 | 근거 | 한계 |
| --- | --- | --- | --- |
| `CursorGeometry` interface | 확정 | `createDOMCursorGeometry` source와 `editorCore.test.ts`의 view adapter path가 `rectForPoint`, `rectsForRange`, `pointFromCoordinates`, `pointForVerticalMovement`, `lineStartPoint`, `lineEndPoint`, `pageStep`을 현재 geometry seam으로 고정한다. | Public editor API로 승격한다는 뜻은 아니다. |
| legal cursor stop rects | 확정 | `cursorGeometry.test.ts`가 rich text, empty paragraph, consecutive empty paragraph, figure, code, wrap fixture의 모든 legal stop에 finite rect를 요구한다. | jsdom fixture 기반 rect contract이며 실제 browser pixel parity는 아니다. |
...
| coordinate branch locality | 확정 현재 상태 | `$doubt if`에서 `cursorGeometry.ts`의 coordinate-to-line point 변환 중복 분기를 기존 `pointFromLineCoordinate` helper로 합쳐 if count를 53개에서 48개로 줄였다. 이후 hard-newline blank-row marker guard가 1개 추가되어 현재 `cursorGeometry.ts`는 if 49/switch 0이고, blank-row regression을 포함해 `cursorGeometry.test.ts` 321개가 통과했다. | 전체 cursor/selection runtime의 모든 `if`가 불필요하다는 뜻은 아니다. Null guard, DOM shape guard, layout edge guard는 입력 방어와 visual row 판정 역할을 가진다. |
