...
| --- | --- | --- |
| plain transfer | markdown처럼 보여도 text insertion이다. | `inputAdapter.test.ts` plain/markdown-looking paste |
| markdown transfer | supported markdown fragment만 marks/link/mention/figure/multi-block/codeBlock으로 복원한다. | `inputAdapter.test.ts`, `clipboard.test.ts` |
...
| 항목 | 판정 | 근거 | 한계 |
| --- | --- | --- | --- |
| current MIME priority | 실행 테스트 확정 | `clipboard.test.ts` | 외부 앱 raw corpus 전체를 대체하지 않는다. |
| `text/uri-list` fallback | 실행 테스트 확정 | `clipboard.test.ts` uri-list cases | Safari/iOS real clipboard event는 #74/browser trace가 필요하다. |
| plain vs markdown mode | 실행 테스트 확정 | `inputAdapter.test.ts`, `BlockEditor.test.tsx` | HTML rich import fidelity는 없음 |
| debug MIME type summary | 실행 테스트 확정 | `debugInteractionSnapshot.test.ts` | raw payload content는 privacy 때문에 report에 넣지 않는다. |
