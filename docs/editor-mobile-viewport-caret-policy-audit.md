...
| 근거 | 내용 |
| --- | --- |
| `src/editor/internal/view/contenteditable/contentEditableSelectionScroll.ts` | 현재 caret reveal은 focused selection target에 `scrollIntoView({ block: "nearest", inline: "nearest" })`만 호출한다. |
...
| 항목 | 판정 | 근거 | 한계 |
| --- | --- | --- | --- |
| focus preserve/reveal 분리 | 실행 테스트로 확정 | `focusScroll.test.ts`, BlockEditor split tests, scroll focus audit가 focus와 reveal을 분리한다. | mobile keyboard occlusion은 닫지 않는다. |
