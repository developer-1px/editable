...
function BlockView({
  ...
  const children = inlineChildrenForRender(block.children).map(
    (child, childIndex) => (
      <InlineView
...
function inlineChildrenForRender(children: InlineNode[]): InlineNode[] {
...
function renderMark(mark: Mark, content: ReactNode): ReactNode {
  ...
  const href = renderableLinkHref(mark.href);
  return (
    <a className="rich-link" href={href} title={mark.title}>
      {content}
    </a>
  );
}
