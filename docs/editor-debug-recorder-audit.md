...
| source guard로 확인 | `INPUT_EVENT_TYPES` 23종 capture list, `document.execCommand("copy")` fallback cleanup details | 구현은 확인되지만 event-list 전체나 fallback selection restore 세부까지 별도 named integration test가 닫지는 않는다. 이 값을 제품 contract로 말하려면 추가 test와 운영 결정을 둬야 한다. |
