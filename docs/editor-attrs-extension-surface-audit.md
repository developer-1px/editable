...
| renderer attrs non-contract | 실행 테스트로 확정 | `DocumentRenderer.test.tsx`가 document/root/block/mention/figure/mark attrs sentinel이 DOM 문자열이나 attribute로 투영되지 않는다고 검증한다. |
| Markdown attrs non-round-trip | 실행 테스트로 확정 | `markdown.test.ts`가 export가 attrs sentinel을 쓰지 않고 import 결과에도 sentinel extension attrs가 복원되지 않는다고 검증한다. Factory-generated typed-field echo attrs는 external attrs syntax가 아니다. |
