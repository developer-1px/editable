...
| `src/editor/internal/model/inputAdapter.ts` | list selection에서 `Tab`/`Shift+Tab`을 list depth command로 보내고, list 밖 plain `Tab`은 text insertion, list 밖 `Shift+Tab`은 selection no-op으로 둔다. |
| `src/editor/internal/model/inputAdapter.test.ts` | Tab block editing policy와 unsupported structural shortcut no-op을 검증한다. |
| `src/editor/internal/model/textCommands.ts` | paragraph/heading/quote/listItem split, codeBlock newline, figure edge insertion 같은 structural editing은 text mutation command가 소유한다. |
...
| JSON Patch shape | 실행 테스트로 확정 | `blockCommands.test.ts`와 `inputAdapter.test.ts`가 `/root/children/{index}/depth` replace patch shape를 검증한다. |
| list Tab/Shift+Tab adapter policy | 실행 테스트로 확정 | `inputAdapter.test.ts`가 list selection에서 `Tab`은 indent, `Shift+Tab`은 outdent로 번역한다고 검증한다. |
| non-list Tab policy | 실행 테스트로 확정 | `inputAdapter.test.ts`가 list 밖 plain `Tab`은 tab text insertion, list 밖 `Shift+Tab`은 selection-only no-op이라고 검증한다. |
