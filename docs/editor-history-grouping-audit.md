...
| 근거 | 의미 |
| --- | --- |
| `editorCore.test.ts` batch dispatch test | 여러 command를 draft에서 평가하고 실제 document에 한 번 commit하므로 undo unit 하나가 된다. |
| `editorCore.test.ts` history command batch rejection test | `undo`/`redo` 같은 direct history command는 patch draft에서 평가하지 않고 batch 전체를 실패시킨다. |
| `editorCore.test.ts` successive single dispatch test | batch가 아닌 연속 headless `insertText` 호출은 각각 undo unit이 된다. |
| `editorCore.test.ts` selection-only test | selection movement는 document history entry를 만들지 않는다. |
| `BlockEditor.test.tsx` history tests | beforeinput history undo/redo 전에 native text edit을 flush하고 selection을 복원한다. blur-flushed native edit도 undo 한 번으로 되돌리고 redo로 복원한다. blur로 끊긴 여러 native edit session은 separate undo unit으로 남는다. |
...
| 판정 대상 | 강도 | 근거 |
| --- | --- | --- |
| explicit command-array batch as one undo unit | 실행 테스트로 확정 | `editorCore.test.ts`가 두 `insertText` command를 한 batch로 dispatch한 뒤 undo 한 번으로 원래 document를 복원하고 redo 한 번으로 batch 결과를 복원한다. |
| history command batch rejection | 실행 테스트로 확정 | `editorCore.test.ts`가 `{ type: "undo" }`가 포함된 batch를 `"History commands cannot be batched."`로 거절하고 document와 undo stack을 보존함을 확인한다. |
| successive single dispatch separation | 실행 테스트로 확정 | `editorCore.test.ts`가 batch가 아닌 두 `insertText` dispatch를 undo 두 번으로 각각 되돌린다. |
| selection-only no document history | 실행 테스트로 확정 | `editorCore.test.ts`가 selection movement 후 `canUndo`가 false임을 확인한다. |
| keyboard/beforeinput history flush | 실행 테스트로 확정 | `BlockEditor.test.tsx`가 active native text edit을 keyboard and beforeinput undo/redo 전에 flush하고 redo로 복원함을 확인한다. |
| native caret restore after history undo | 실행 테스트로 확정 | `BlockEditor.test.tsx`가 observed native range에서 history undo 후 native collapsed caret과 overlay가 canonical selection으로 복원됨을 확인한다. |
| blur-flushed native edit undo unit | 실행 테스트로 확정 | `BlockEditor.test.tsx`가 blur로 flush된 native text edit을 undo 한 번으로 되돌리고 redo로 복원한다. |
| separate blur-flushed sessions | 실행 테스트로 확정 | `BlockEditor.test.tsx`가 blur로 끊긴 두 native edit session이 자동 merge되지 않고 undo 두 번으로 각각 되돌아감을 확인한다. |
