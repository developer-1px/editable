...
describe("document normalizer", () => {
  ...
  it("preserves document, root, block, and atom attrs while normalizing children", () => {
    const normalized = normalizeDocument({
      ...documentWithBlocks([
        {
          id: "block-1",
      ...
      attrs: { owner: "team-editor" },
      root: {
        ...documentWithBlocks([]).root,
        attrs: { outline: true },
        children: [
          {
            id: "block-1",
  ...
  it("drops mark attrs while preserving link target fields", () => {
    expect(
      normalizeInlineChildren([
        {
          type: "text",
