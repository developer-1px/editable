...
export function moveVisualLeft(
  document: NoteDocument,
  selection: SelectionSnap,
  geometry: CursorGeometryAdapter,
  options: CursorMoveOptions = {},
): CursorCommandResult {
  return moveHorizontalWithGeometry(
    document,
    selection,
    geometry,
    "backward",
...
export function moveVisualRight(
  document: NoteDocument,
  selection: SelectionSnap,
  geometry: CursorGeometryAdapter,
  options: CursorMoveOptions = {},
  ...
  if (rect === null) {
    ...
    return {
      selectionAfter: selectionAfterMove(
        ...
        target,
        options,
        contextWithPreferredX(selection.context, 0),
      ),
    };
  }
  ...
  if (target === null) {
    ...
    return {
      selectionAfter: selectionAfterMove(
        ...
        fallback,
        options,
        contextWithPreferredX(selection.context, preferredX),
      ),
    };
  }
...
function moveHorizontalWithGeometry(
  document: NoteDocument,
  selection: SelectionSnap,
  geometry: CursorGeometryAdapter,
  direction: "backward" | "forward",
  ...
  const visualTarget = geometry.pointForHorizontalMovement?.(
    current,
    direction,
  );

  if (visualTarget !== undefined && visualTarget !== null) {
    return {
      selectionAfter: selectionAfterMove(
        document,
        selection,
        normalizeCursorPoint(document, visualTarget),
        options,
      ),
    };
  }

  return moveHorizontal(document, selection, direction, options);
}
...
function contextWithPreferredX(
  context: SelectionContext | undefined,
  preferredX: number,
): SelectionContext {
  const record = contextRecord(context);
  if (record !== null) {
    return { ...record, preferredX } as SelectionContext;
  }
