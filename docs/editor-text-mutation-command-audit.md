...
| 근거 | 내용 |
| --- | --- |
| `src/editor/internal/model/text-command/textCommands.ts` | `insertText`, `insertMention`, `insertInlineFragment`, `insertBlockFragment`, `insertFigure`, delete variants, `splitParagraph`가 document/selection을 patch와 `selectionAfter`로 변환한다. |
| `src/editor/internal/model/text-command/textCommandAddressing.ts` | JSON pointer string을 inline text, code text, inline atom, block atom 위치로 파싱한다. |
| `src/editor/internal/model/text-command/textCommandSelection.ts` | inline/block removal과 split/merge 뒤 deterministic selection placement를 계산한다. |
| `src/editor/internal/model/text-command/textCommandAtomDeletion.ts` | inline/block atom whole-removal patch와 replacement/deletion 뒤 selection placement를 계산한다. |
| `src/editor/internal/model/editorCommandStrategies.ts` | public `insertNode`와 `delete` command를 text command strategy로 수렴시킨다. |
| text command split tests | `textCommandInsertion.test.ts`, `textCommandEdgeInsertion.test.ts`, `textCommandDeletion.test.ts`, `textCommandWordDeletion.test.ts`, `splitParagraph.test.ts`, `textCommandMentionInsertion.test.ts`, `textCommandFigureInsertion.test.ts`, `textCommandHistory.test.ts`, `textCommandResultContract.test.ts`가 insertion, range replacement, grapheme/word deletion, atom deletion/replacement, code-aware range replacement, split/merge, fragment insertion, undo/redo selection restore를 검증한다. |
| inputAdapter split tests | browser beforeinput/key input이 current text command paths로 번역되는 것을 검증한다. |
...
| grapheme/word deletion | character deletion은 grapheme boundary를 지키고, word deletion은 `textCommandWordDeletion.ts`가 movement command로 만든 extended range를 기존 delete path에 위임해 삭제한다. |
...
| figure/block edge behavior | inline atom edge insertion은 inline child list에 text node를 끼우고, block/text-block edge insertion은 인접 paragraph/code block을 쓰거나 새 paragraph block을 만든다. |
...
| 항목 | 강도 | 이유 |
| --- | --- | --- |
| command result contract | 실행 테스트로 확정 | text command split tests가 insert/delete/split이 input document를 직접 mutate하지 않고 patch와 `selectionAfter`를 반환하는 것을 고정한다. |
| text/code leaf editing | 실행 테스트로 확정 | paragraph/heading/listItem inline text와 codeBlock text path가 같은 text command path로 처리되는 것을 text command split tests가 검증한다. |
| structured mark preservation | 실행 테스트로 확정 | ordinary insert/delete가 existing marks를 보존하고, collapsed active marks insert가 marked text run을 만드는 경로는 text command split tests와 `mark command split tests`가 닫는다. |
...
| block merge and figure/block edge behavior | 실행 테스트로 확정 | paragraph start/end merge, figure edge text insertion/split, isolated figure paragraph creation을 text command split tests와 regression tests가 덮는다. Block-side edge insertion은 `textCommandBlockEdgeInsertion.ts`, shared inline text child add primitive는 `textCommandInlineTextInsertion.ts`에 있다. |
...
| input adapter convergence | 실행 테스트로 확정 | beforeinput/key input의 insert/delete/split/paste/drop path는 inputAdapter split tests와 BlockEditor split tests가 같은 model command path로 수렴하는지 검증한다. |
...
| `textCommandWordDeletion` adapter | 유지 확정 | word movement와 deletion patch construction을 섞지 않고, movement가 만든 selected range를 existing delete command에 위임한다. |
| `textCommandBlockEdgeInsertion` helper | 유지 확정 | block/text-block edge insertion은 block sequence와 code/paragraph backing text patch를 다루므로 inline atom child-list insertion과 분리한다. |
...
| file split by every helper function | 보류 | 실제 독립 변경 이유가 확인된 atom deletion primitive처럼 shared responsibility는 분리한다. 단순 file-size 분할이나 helper 함수별 분할은 하지 않는다. |
...
| file-level refactor policy | `textCommands.ts`는 facade가 되었고, 현재는 독립 변경 이유가 확인된 text mutation helpers만 internal modules로 분리한다. | 실제 독립 변경 이유가 반복되면 range replacement, atom replacement, fragment splice 등 internal modules를 더 조정할지 검토한다. |
