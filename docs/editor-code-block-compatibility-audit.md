...
| `src/editor/internal/model/text-command/textCommandAddressing.ts` | code block text cursor path는 `/root/children/{index}/text`다. Inline child path인 `/children/{childIndex}/text`를 code block content path로 쓰지 않는다. |
...
| `note document split tests` | `children`이 있는 code block을 schema가 받지만 `readBlockText`는 canonical `text`를 읽는 것을 검증한다. |
| `markdown split tests` | Markdown export가 compatibility `children`이 아니라 `text` field를 쓰는 것을 검증한다. |
| `DocumentRenderer split tests` | renderer가 compatibility `children`을 DOM에 렌더하지 않고 `text` field만 렌더하는 것을 검증한다. |
...
| code block defaulting | 실행 테스트로 확정 | `note document split tests`가 `text`와 `children`이 없는 code block을 parse하면 `text: ""`, `children: []`로 default 된다고 검증한다. |
| children compatibility preservation | 실행 테스트로 확정 | `note document split tests`가 legacy `children` payload를 schema-valid data로 보존하면서 `readBlockText`는 `text`를 읽는다고 검증한다. |
...
| renderer children non-contract | 실행 테스트로 확정 | `DocumentRenderer split tests`가 `legacy child`를 렌더하지 않고 `text` field만 code DOM에 렌더한다고 검증한다. |
| Markdown children non-round-trip | 실행 테스트로 확정 | markdown split tests가 compatibility `children` 대신 fenced code `text`와 optional `language`만 export한다고 검증한다. |
