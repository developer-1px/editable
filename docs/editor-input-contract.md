...
| SEL-01 | selection | collapsed selection + `keydown ArrowLeft/ArrowRight` | 한 cursor unit 이동 | document mutation 없음 | 이전/다음 valid cursor point | caret overlay만 이동 | cursor command tests | `selection/collapsed-horizontal` |
| SEL-02 | selection | non-collapsed range + plain `ArrowLeft/ArrowRight` | range collapse | document mutation 없음 | ArrowLeft는 range start, ArrowRight는 range end로 collapse. 추가 이동 없음 | range overlay 제거, caret 표시 | editor native expectation | `selection/range-collapse-horizontal` |
| SEL-03 | selection | collapsed/range + `Shift+ArrowLeft/ArrowRight` | anchor를 유지하고 focus를 한 unit 확장/축소 | document mutation 없음 | direction 포함 range selection | range overlay가 canonical range만 표시 | cursor command tests | `selection/shift-horizontal-range` |
...
| MUT-01 | text replacement | non-collapsed range + printable key or `beforeinput insertText` | selected content replace | 선택 범위를 삭제하고 입력 text 삽입 | inserted text 끝 collapsed caret | range overlay 제거, native text leaf reset | text command tests | `mutation/range-insert-text` |
...
| DEL-02 | deletion | non-collapsed range + Backspace/Delete or `beforeinput deleteContent` | selected content delete | range content 삭제 | range start collapsed caret | range overlay 제거 | deletion command tests | `deletion/range` |
| DEL-03 | deletion | empty block boundary + Backspace/Delete | boundary merge/delete policy | 허용된 경우 paragraph merge 또는 empty block 제거, 아니면 no-op | merge/delete 후 deterministic caret | 빈 block caret rect 유지 | paragraph merge tests, cursor geometry tests | `deletion/empty-block-boundary` |
| CLIP-01 | clipboard/drop | paste event or `beforeinput insertFromPaste` with plain text | plain text paste | selection replace 또는 caret insert | pasted text 끝 | native DOM paste prevented or reconciled | clipboard/input adapter tests | `clipboard/plain-paste` |
| CLIP-02 | clipboard/drop | custom MIME markdown or external `text/markdown` paste/drop | supported rich fragment paste | marks/link/mention/figure/multi-block 중 지원 범위 복원 | inserted fragment 끝 또는 deterministic block point | renderer-owned DOM only | clipboard markdown tests | `clipboard/markdown-fragment` |
| CLIP-03 | clipboard/drop | `Cmd/Ctrl+X` or `beforeinput deleteByCut` | copy selection then delete through command | selection serialized, selected content deleted | deletion selectionAfter | clipboard side effect와 model mutation 순서 고정 | cut tests | `clipboard/cut-delete` |
